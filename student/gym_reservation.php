...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if the time slot is available - use facility_type instead of facility_id
    $check_query = "SELECT * FROM bookings 
                   WHERE facility_type = 'gym'
                   AND (booking_date = ? OR date = ?)
        ...
        $stmt->bind_param("ssssssss", $booking_date, $booking_date, $start_time, $start_time, $end_time, $end_time, $start_time, $end_time);
        $stmt->execute();
        ...
        if ($result->num_rows > 0) {
            ...
            // Insert the booking - try with user_type first, fallback to facility_type
            $insert_success = false;
            
            // First try: with user_type and booking_date (like external/gym_reservation.php)
            ...
            // Fallback: with facility_type and date, no facility_id (like external/process_gym_booking.php)
            if (!$insert_success) {
                $insert_query = "INSERT INTO bookings (user_id, facility_type, date, start_time, end_time, purpose, participants, status, created_at) 
                                VALUES (?, 'gym', ?, ?, ?, ?, ?, 'pending', NOW())";
                $stmt = $conn->prepare($insert_query);
                if ($stmt) {
                    $stmt->bind_param("issssi", $user_id, $booking_date, $start_time, $end_time, $purpose, $participants);
                    if ($stmt->execute()) {
                        $insert_success = true;
                    }
                }
            ...
            if ($insert_success) {
                ...
                create_notification($user_id, "Gym Reservation Submitted", "Your gym reservation for {$date_formatted} has been submitted and is pending approval.", "booking", "student/gym_reservation.php");
...
// Get user's existing reservations - simple query without joins (like external/dashboard.php)
$reservations_query = "SELECT b.*, 'Gym Facility' as facility_name 
                      FROM bookings b
                                ...
                                <?php if ($reservations->num_rows > 0): ?>
                                    <?php while ($reservation = $reservations->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <?php 
                                                $reservation_date = $reservation['booking_date'] ?? $reservation['date'] ?? null;
                                                if ($reservation_date) {
                                                    echo date('M d, Y', strtotime($reservation_date));
                                                } else {
                                                    echo 'N/A';
                                                }
                                                ?>
